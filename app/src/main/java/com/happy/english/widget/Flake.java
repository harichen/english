...
public class Flake {
     ...
     * @param locationList 
     */
    static Flake createFlake(int index ,float xRange, Bitmap originalBitmap, ArrayList<Point> locationList) {
        Flake flake = new Flake();
        // Size each flake with a width between 5 and 55 and a proportional height
//        flake.width = (int)(50 + (float)Math.random() * 250);
...
//        flake.x = (float)Math.random() * (xRange - flake.width);
        flake.x = locationList.get(index).x ;
        flake.y = locationList.get(index).y ;
        // Position the flake vertically slightly off the top of the display
//        flake.y = 400 + ((float)Math.random() * 1000);

        // Each flake travels at 50-200 pixels per second
        flake.speed = 20 + (float) Math.random() * 20;
